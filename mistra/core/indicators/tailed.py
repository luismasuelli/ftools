class TailedMixin:
    """
    Adds capability to extract a "tail" of values from an underlying source.
    It adds the capability to have a tail size and get chunks of data of that
      particular size, iteratively (mostly intended for "moving" indicators,
      retrieves a backward-expanding "tail" of values that may be, or not,
      complete in size).

    This mixin must be initialized after the "regular" indicator, to ensure
      the data is available to it.
    """
    ...
    def _tail_iterator(self, start, end, tail_source):
        """
        ...
        :param tail_source: The source of the iterator.
        ...
        data = self._tail_slice(tail_source, start, end, self._tail_size)
